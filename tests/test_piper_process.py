...
from ai_contained.provider.aws_cli.piper_process import PiperProcess, PiperResponse
...
def describe_PiperProcess():
    ...
    def describe_wait():
        async def it_captures_stdout(env) -> None:
            expected = PiperResponse(exit_code=0, stdout="hello", stderr="", is_truncated=False)
            async with PiperProcess(py(f"print('{expected['stdout']}', end='')"), env=env) as proc:
                result = await proc.wait()
            assert_that(result).is_equal_to(expected)

        async def it_captures_stderr(env) -> None:
            expected = PiperResponse(exit_code=0, stdout="", stderr="something went wrong", is_truncated=False)
        ...
        async def it_captures_stdout_and_stderr(env) -> None:
            expected = PiperResponse(exit_code=0, stdout="out", stderr="err", is_truncated=False)
        ...
        async def it_reflects_non_zero_exit_code(env) -> None:
            expected = PiperResponse(exit_code=42, stdout="", stderr="", is_truncated=False)
        ...
        async def it_handles_empty_output(env) -> None:
            expected = PiperResponse(exit_code=0, stdout="", stderr="", is_truncated=False)
    ...
    def describe_stdout():
        ...
        async def it_relays_output_to_downstream_stdin(env) -> None:
            ...
            assert_that(downstream_result).is_equal_to(expected)

        async def it_fills_buffer_even_when_downstream_fails(env) -> None:
            expected_upstream = PiperResponse(exit_code=0, stdout="hello", stderr="", is_truncated=False)
            expected_downstream = PiperResponse(exit_code=1, stdout="", stderr="", is_truncated=False)
            async with PiperProcess(py(f"print('{expected_upstream['stdout']}', end='')"), env=env) as upstream:
            ...
            assert_that(downstream_result).is_equal_to(expected_downstream)
